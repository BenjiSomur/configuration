...
with open("./{}/changed.csv".format(filename), 'w') as f:
    f.write('No Gen., Selected, Mutated\n')
...
while no_gen < gens:
    ...
    for i in range(0, len(parents), 2):
        ...
        if chld1[0] == 'selected':
            mselected += 1
            if chld1[1][0] == 'changed':
                mchanged += 1
        if chld2[0] == 'selected':
            mselected += 1
            if chld2[1][0] == 'changed':
                mchanged += 1
        offsp += [chld1[1][1], chld2[1][1]]
    aux = list()
    for chrom in offsp:
        ft = fitness(chrom)
        aux.append([chrom, ft])
    ...
    with open("./{}/changed.csv".format(filename), 'a') as f:
        f.write("{},{},{}".format(no_gen, mselected, mchanged))
    no_gen += 1
