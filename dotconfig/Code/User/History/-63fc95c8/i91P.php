...
if (isset($_POST['soleval']) || isset($_POST['enviararchivo'])) {
    ...
    if (is_null($no_sol)) {
        ...
        $aux->getElementById('mensajenoregistro')->delete();
...
elseif (isset($_POST['entrevistanull'])) {
    ...
    $msghtml = new TextNode("<h2 style='color: #999999'><b style='font-size: 40px'>$message</b></h2>");
...
elseif (isset($_POST['acreditacion'])) {
    ...
    if (is_null($acreditacion)) {
        $msghtml = new TextNode("<h2 style='color: #999999'><b style='font-size: 40px'>La evaluación no ha concluido, <br>favor de intentarlo de nuevo más tarde</b></h2>");
        $jumbotron->addChild($msghtml);

    } else {
        $msghtml = new TextNode("<h2 style='color: #53565a'><b style='font-size: 40px'>Periodo de evaluación concluído</b></h2><hr>");
        $jumbotron->addChild($msghtml);
        ...
        if ($acreditacion == 1) {
            $tipodistintivo = new TextNode("<h1 style='color: #ddaf42'><b> ORO</b> </h1>");
        } elseif ($acreditacion == 2) {
            $tipodistintivo = new TextNode("<h1 style='color: #9eaaac'><b> PLATA</b> </h1>");
        } else {
            $tipodistintivo = new TextNode("<h1 style='color: #dc954d'><b> BRONCE</b> </h1>");
        }
