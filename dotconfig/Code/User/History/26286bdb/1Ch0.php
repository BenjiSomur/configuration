...
echo $phpWord->getSettings()->getThemeFontLang() . "<br/>";

$phpWord->setDefaultFontName('calibri');

$linkstyle = 'rStyle';

$phpWord->addLinkStyle($linkstyle, ['name' => 'calibri', 'size' => 8]);
        ...
        if ($element instanceof TextRun) {
            foreach ($element->getELements() as $aux) {
                if ($aux instanceof Link) {
                    $aux->getFontStyle()->setColor('#0000ff')
                        ->setUnderline('single')->setSize(5);
                }
            }
        }
