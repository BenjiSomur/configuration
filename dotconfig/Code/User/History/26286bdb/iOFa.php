...
$phpWord->getSettings()->getDocumentProtection()->setEditing(\PhpOffice\PhpWord\SimpleType\DocProtect::)
        ...
        if ($element instanceof TextRun) {
            foreach ($element->getELements() as $aux) {
                if ($aux instanceof Link) {
                    $aux->getFontStyle()->setSize(8);
                    foreach ($aux->getFontStyle()->getStyleValues()['basic'] as $val) {
                        echo $val . "<br/>";
                    }
                    echo $aux->getFontStyle()->getStyleValues()['name'] . "<br/>";
                    foreach ($aux->getFontStyle()->getStyleValues()['style'] as $val) {
                        echo $val . "<br/>";
                    }
                }
            }
...
$objWriter->save("$pathname" . "_test.docx");
